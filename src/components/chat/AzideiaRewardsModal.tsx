...
export default function AzideiaRewardsModal({
  ...
  const availableBarracoTime = Math.max(
    0,
    Number(status?.available?.barraco_time ?? 0),
  );
  const totalAvailable = availableTwoX + availableCorre + availableBarracoTime;
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const claimedBarracoTime = Math.max(
        0,
        Number(result.claimed?.barraco_time ?? 0),
      );
      const parts: string[] = [];
      if (claimedTwoX > 0)
        parts.push(`${claimedTwoX} acelerador(es) de comboio`);
      if (claimedCorre > 0) parts.push(`${claimedCorre} Corre(s)`);
      if (claimedBarracoTime > 0)
        parts.push(
          `${formatSeconds(claimedBarracoTime)} de acelerador de barraco`,
        );
      setClaimedMessage(
        parts.length
          ? `Você coletou ${parts.join(" e ")}.`
      ...
      <div className="w-full max-w-2xl rounded-3xl border border-red-500/40 bg-zinc-950 p-5 shadow-2xl">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl bg-zinc-900 px-3 py-2 text-sm font-black text-zinc-300"
          >
            Fechar
          </button>
        </div>
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
                ...
                <div className="flex items-center gap-3 rounded-2xl bg-zinc-900 p-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-zinc-500">
                      Acelerador de barraco
                    </p>
                    <p className="text-3xl font-black text-amber-300">
                      {formatSeconds(availableBarracoTime)}
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-3 space-y-2 rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-zinc-400">
                <p>
                  X9: coleta limitada a {status?.x9FactionDailyLimit ?? 100}{" "}
                  aceleradores por dia. Hoje você já coletou{" "}
                  {status?.x9FactionReceivedToday ?? 0}.
                </p>
                <p>
                  Correria: coleta limitada a{" "}
                  {status?.correriaFactionDailyLimit ?? 100} Corres por dia.
                  Hoje você já coletou{" "}
                  {status?.correriaFactionReceivedToday ?? 0}.
                </p>
                <p>
                  Mestre de Obras: coleta limitada a{" "}
                  {status?.mestreObrasFactionDailyLimit ?? 100} aceleradores de
                  barraco por dia. Hoje você já coletou{" "}
                  {status?.mestreObrasFactionReceivedToday ?? 0}.
                </p>
              </div>
                  ...
                  <div className="space-y-2">
                    {status.batches.map((batch) => {
                      const rewardType = String(
                        batch.rewardType || "convoy_2x",
                      );
                      const actionText =
                        rewardType === "corre"
                          ? "negociou Correria"
                          : rewardType === "barraco_time"
                            ? "pagou Mestre de Obras"
                            : "eliminou X9";
                      const rewardText =
                        rewardType === "corre"
                          ? `${batch.quantity} Corre`
                          : rewardType === "barraco_time"
                            ? `${formatSeconds(batch.quantity)} de acelerador de barraco`
                            : `${batch.quantity} acelerador`;
                      return (
                        <div
                          key={batch.id}
                          className="rounded-xl bg-black/30 px-3 py-2 text-xs text-zinc-300"
                        >
        ...
        {claimedMessage && (
          <p className="mt-3 rounded-2xl bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">
            {claimedMessage}
          </p>
        )}
        {error && (
          <p className="mt-3 rounded-2xl bg-red-500/10 px-3 py-2 text-sm text-red-300">
            {error}
          </p>
        )}
      </div>
