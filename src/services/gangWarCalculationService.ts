...
export function applyFormationToGangStats(
  ...
  const next: GangBattleCompositionStats = {
    ...
    totalPower:   stats.totalPower, // recalculated below
  };
...
export function buildGangBattleStatsWithFormation(
  members: GangUnit[],
  formation: GangFormationType
): GangBattleCompositionStats {
  const base = buildGangBattleCompositionStats(members);
  return applyFormationToGangStats(base, formation);
}
