...
const AZIDEIA_MESTRE_OBRAS_ICON_EMOJI = "🏗️";
...
const FactionHelpCard = memo(
  ({
    ...
    const alreadyHelped = (request.helperIds || []).includes(
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-black text-white text-sm">
              {request.requesterName} pediu corre
            </p>
...
const AzideiaRewardCard = memo(({ message }: { message: ChatMessage }) => {
  ...
  const accent = isMestreObras ? "amber" : isCorreria ? "emerald" : "red";
  const title = isMestreObras
    ? "Mestre de Obras"
    : isCorreria
      ? "Correria"
      : "Azidéia";
  const text = isMestreObras
    ? `${killerName} pagou um Mestre de Obras. A facção recebeu aceleradores de barraco para coletar.`
    : isCorreria
  ...
  const reward = isMestreObras
    ? "+5min de acelerador de barraco"
    : isCorreria
      ? "+1 Corre"
      : "+1 acelerador de comboio";

  return (
    <div
      className={`rounded-2xl border ${accent === "amber" ? "border-amber-500/30" : accent === "emerald" ? "border-emerald-500/30" : "border-red-500/30"} bg-black/60 p-4`}
    >
      <div className="flex items-center gap-3">
        {isMestreObras ? (
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 text-4xl">
        ...
        <div className="min-w-0 flex-1">
          <p
            className={`text-sm font-black uppercase tracking-wide ${accent === "amber" ? "text-amber-200" : accent === "emerald" ? "text-emerald-200" : "text-red-200"}`}
          >
            {title}
          </p>
          <p className="mt-0.5 text-sm text-white">{text}</p>
          <p className="mt-1 text-[11px] text-zinc-500">
            Recompensa: {reward} por membro
            {totalMembers > 0 ? ` • ${totalMembers} membros` : ""}
        ...
        <button
          type="button"
          onClick={openRewards}
          className={`shrink-0 rounded-xl px-3 py-2 text-xs font-black uppercase text-white ${accent === "amber" ? "bg-amber-600" : accent === "emerald" ? "bg-emerald-600" : "bg-red-600"}`}
        >
          Coletar
        </button>
      </div>
  ...
  ({
    ...
    return (
      ...
      >
        ...
        {!isMine && (
          <div
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-black text-white ${avatarColor(message.senderName)}`}
          >
        ...
        >
          ...
          >
            ...
            <div
              className={`mt-1 text-[10px] ${isMine ? "text-right text-red-200" : "text-zinc-500"}`}
            >
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-black text-white ${avatarColor(otherName)}`}
            >
