...
import { X, Copy, CheckCircle2, AlertTriangle, Loader2, Zap } from 'lucide-react';
import {
  createMercadoPagoBrickCorrePackagePayment,
...
declare global {
  interface Window {
    MercadoPago?: any;
    __commandiaCorrePaymentBrickController?: any;
  }
}
...
export default function CorrePackagePaymentModal({ pack, open, onClose }: Props) {
  const containerId = useMemo(() => `mp-payment-brick-corre-${pack?.id || 'package'}`, [pack?.id]);
  ...
  useEffect(() => {
    ...
    async function mountBrick() {
      try {
        if (window.__commandiaCorrePaymentBrickController?.unmount) {
          window.__commandiaCorrePaymentBrickController.unmount();
          window.__commandiaCorrePaymentBrickController = null;
        }
          ...
          callbacks: {
            ...
            onSubmit: ({ selectedPaymentMethod, formData }: any) => {
              ...
              return new Promise<void>(async (resolve, reject) => {
                try {
                  const response = await createMercadoPagoBrickCorrePackagePayment(pack.id, {
                    ...formData,
            ...
            onError: (brickError: any) => {
              console.error('[MercadoPago Corre Brick]', brickError);
              if (!mountedRef.current) return;
        ...
        window.__commandiaCorrePaymentBrickController = await bricksBuilder.create('payment', containerId, settings);
      } catch (err: any) {
        console.error('[CorrePackagePaymentModal]', err);
        if (!mountedRef.current) return;
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/80 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-t-[2rem] sm:rounded-3xl border border-yellow-400/30 bg-zinc-950 text-white shadow-[0_0_40px_rgba(250,204,21,0.18)]">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-zinc-950/95 p-4">
          <div>
            ...
            <p className="text-sm text-white/60">
              +{pack.correAmount} Corres · {formatBRL(pack.price)} · cartão, Pix ou boleto no mesmo fluxo do comboio
            </p>
          </div>
        ...
        <div className="space-y-4 p-4">
          <div className="flex items-center gap-3 rounded-2xl border border-yellow-400/25 bg-yellow-400/10 p-4 text-sm text-yellow-100">
            ...
            <div>
              ...
              <div>Quando o pagamento aprovar, entram +{pack.correAmount} Corres direto no saldo do jogador.</div>
            </div>
          </div>
          ...
          {state === 'approved' && (
            <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-emerald-100">
              ...
              <div>
                ...
                <div>+{pack.correAmount} Corres foram adicionados na sua conta.</div>
              </div>
            </div>
          )}
          ...
          {state === 'pending' && (
            <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-4 text-sm text-yellow-100">
              ...
              <div>Se for Pix ou boleto, os Corres entram automaticamente quando o Mercado Pago confirmar.</div>
            </div>
          )}
