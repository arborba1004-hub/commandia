...
function generateSpinResult(multiplier: number): SpinResult {
  ...
  if (policeCount === 3) {
    return {
      ...
      prison: true,
      doublePolice: false,
      label: '🚔 A casa caiu. Tu rodou pesado.',
    };
  }
  ...
  if (diamondCount === 3) {
    return {
      reels,
      dirtyGain: 10000 * multiplier,
      prison: false,
      doublePolice: false,
      label: `💎 JACKPOT! +${(10000 * multiplier).toLocaleString('pt-BR')} Commands Sujo`,
    };
  }
  ...
  if (moneyCount === 3) {
    return {
      reels,
      dirtyGain: 2500 * multiplier,
      prison: false,
      doublePolice: false,
      label: `💵 Trinca pesada! +${(2500 * multiplier).toLocaleString('pt-BR')} Commands Sujo`,
    };
  }
  ...
  if (gunCount === 3) {
    return {
      reels,
      dirtyGain: 1600 * multiplier,
      prison: false,
      doublePolice: false,
      label: `🔫 Corre bruto! +${(1600 * multiplier).toLocaleString('pt-BR')} Commands Sujo`,
    };
  }
  ...
  if (policeCount === 2) {
    return {
      ...
      prison: false,
      doublePolice: true,
      label: '👀 Olha ozomi... os homens quase te pegaram.',
    };
  }
  ...
  if (moneyCount === 2) {
    return {
      reels,
      dirtyGain: 700 * multiplier,
      prison: false,
      doublePolice: false,
      label: `💵 Caiu bem. +${(700 * multiplier).toLocaleString('pt-BR')} Commands Sujo`,
    };
  }
  ...
  if (diamondCount === 2) {
    return {
      reels,
      dirtyGain: 1200 * multiplier,
      prison: false,
      doublePolice: false,
      label: `💎 Quase monstro. +${(1200 * multiplier).toLocaleString('pt-BR')} Commands Sujo`,
    };
  }

  return {
    reels,
    dirtyGain: 120 * multiplier,
    prison: false,
    doublePolice: false,
        ...
        {doublePoliceOpen && (
          ...
          >
            <div className="rounded-[26px] border border-yellow-400/30 bg-[#11141a] px-8 py-6 shadow-[0_0_50px_rgba(255,220,0,0.22)]">
              <h2 className="text-center text-3xl md:text-5xl font-black uppercase tracking-[0.16em] text-yellow-300">
                OLHA OZOMI
              </h2>
              <p className="mt-3 text-center text-sm md:text-base uppercase tracking-[0.18em] text-zinc-200">
                Os homens tão rondando o corre
              </p>
            </div>
          </motion.div>
        )}
        ...
        {jackpotOpen && (
          ...
          >
            ...
            <div className="relative text-center">
              <h2 className="text-5xl md:text-7xl font-black uppercase tracking-[0.18em] text-yellow-300 drop-shadow-[0_0_18px_rgba(255,230,120,0.7)]">
                JACKPOT
              </h2>
              <p className="mt-4 text-xl md:text-3xl font-bold text-white">
                💎 O asfalto sorriu pra tu
              </p>
            </div>
          </motion.div>
        )}
