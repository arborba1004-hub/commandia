...
export default function GiroPage() {
  ...
  const finalizeSpin = (result: SpinResult) => {
    if (result.prison) {
      ...
      addHistory(`🚔 Prisão: ${result.label}`);
      triggerPrison();
      setSpinning(false);
      return;
    }
    ...
    if (result.reels.every((symbol) => symbol === 'diamond')) {
      triggerJackpot();
    }
                ...
                <div className="rounded-[28px] border border-yellow-500/35 bg-black/72 p-4 shadow-[0_0_35px_rgba(255,200,0,0.15)] backdrop-blur-md">
                  ...
                  {spinError && (
                    <div className="mt-3 rounded-2xl border border-red-500/30 bg-red-500/10 px-3 py-3 text-center">
                      <span className="text-[11px] md:text-sm font-semibold text-red-300">
                        {spinError}
                      </span>
                    </div>
                  )}
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4">
                ...
                <p className="mt-2 text-sm leading-relaxed text-zinc-200">
                  O resultado do giro é decidido pelo backend. Esta página só anima
                  os rolos, exibe o resultado e hidrata o estado atualizado do jogador.
                </p>
              </div>
            </div>
          </div>
