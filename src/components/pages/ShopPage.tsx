...
import { ChevronLeft, Clock, ShieldCheck, ShoppingCart, Sparkles, Zap } from 'lucide-react';
...
import BarracoAcceleratorPackagePaymentModal from '@/components/payments/BarracoAcceleratorPackagePaymentModal';
import { BARRACO_ACCELERATOR_STARTER_PACKAGE, CORRE_STARTER_PACKAGE, formatBRL } from '@/api/shopApi';
...
export default function ShopPage() {
  ...
  const [barracoAcceleratorPaymentOpen, setBarracoAcceleratorPaymentOpen] = useState(false);

  const barracoUnitDuration = formatDuration(BARRACO_ACCELERATOR_STARTER_PACKAGE.secondsPerAccelerator);
  const barracoTotalDuration = formatDuration(BARRACO_ACCELERATOR_STARTER_PACKAGE.totalSeconds);
          ...
          <div className="space-y-10">
            <div className="text-center">
              ...
              <p className="mx-auto mt-2 max-w-2xl text-sm text-white/60">
                Pacotes com pagamento embutido no jogo via Payment Brick, sem mandar o jogador para checkout externo.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="group relative overflow-hidden rounded-[2rem] border border-orange-400/40 bg-[radial-gradient(circle_at_20%_0%,rgba(251,146,60,0.28),transparent_34%),linear-gradient(135deg,rgba(33,15,5,0.96),rgba(5,5,5,0.98))] p-6 shadow-[0_0_38px_rgba(251,146,60,0.16)]">
                <div className="pointer-events-none absolute -right-12 -top-12 h-36 w-36 rounded-full bg-orange-300/20 blur-2xl transition group-hover:bg-orange-300/30" />
                <div className="pointer-events-none absolute left-5 top-5 h-20 w-20 rounded-full border border-orange-300/30 opacity-40 animate-ping" />

                <div className="relative z-10 flex items-start justify-between gap-3">
                  <div className="inline-flex items-center gap-2 rounded-full border border-orange-300/30 bg-orange-300/15 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-orange-100">
                    <Sparkles className="h-3.5 w-3.5" />
                    {BARRACO_ACCELERATOR_STARTER_PACKAGE.badge}
                  </div>
                  <div className="rounded-full border border-yellow-400/30 bg-yellow-400/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-yellow-100">
                    Brick
                  </div>
                </div>

                <div className="relative z-10 mt-8 flex items-center justify-center">
                  <div className="relative flex h-36 w-36 items-center justify-center rounded-[2rem] border border-orange-300/30 bg-black/65 shadow-[inset_0_0_24px_rgba(251,146,60,0.10),0_0_32px_rgba(251,146,60,0.20)]">
                    <Clock className="h-20 w-20 text-orange-300 drop-shadow-[0_0_18px_rgba(251,146,60,0.55)]" />
                    <div className="absolute -bottom-3 rounded-full border border-orange-300/40 bg-black px-4 py-1 text-xl font-black text-orange-300">
                      x{BARRACO_ACCELERATOR_STARTER_PACKAGE.acceleratorCount}
                    </div>
                  </div>
                </div>

                <div className="relative z-10 mt-8">
                  <h3 className="font-heading text-2xl font-black text-white">{BARRACO_ACCELERATOR_STARTER_PACKAGE.name}</h3>
                  <p className="mt-2 min-h-[44px] text-sm leading-relaxed text-white/60">
                    {BARRACO_ACCELERATOR_STARTER_PACKAGE.description}
                  </p>
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-3">
                    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                      ...
                      <div className="mt-1 text-lg font-black text-orange-300">
                        {BARRACO_ACCELERATOR_STARTER_PACKAGE.acceleratorCount} de {barracoUnitDuration}
                      </div>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                      <div className="text-[10px] font-black uppercase tracking-[0.16em] text-white/35">Preço</div>
                      <div className="mt-1 text-lg font-black text-[#d9b764]">{formatBRL(BARRACO_ACCELERATOR_STARTER_PACKAGE.price)}</div>
                    </div>
                  </div>

                  <div className="mt-4 flex items-start gap-2 rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-3 text-xs leading-relaxed text-cyan-100">
                    <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
                    Entrega automática em barracoAccelerators.seconds: {barracoTotalDuration} para acelerar obras do barraco.
                  </div>

                  <button
                    type="button"
                    onClick={() => setBarracoAcceleratorPaymentOpen(true)}
                    className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(180deg,#fb923c_0%,#9a3412_100%)] px-4 py-4 font-black uppercase tracking-[0.14em] text-black shadow-[0_0_26px_rgba(251,146,60,0.25)] transition hover:brightness-110 active:scale-[0.98]"
                  >
                    <ShoppingCart className="h-5 w-5" />
            ...
            />
            <BarracoAcceleratorPackagePaymentModal
              pack={BARRACO_ACCELERATOR_STARTER_PACKAGE}
              open={barracoAcceleratorPaymentOpen}
              onClose={() => setBarracoAcceleratorPaymentOpen(false)}
            />
          </div>
        )}
